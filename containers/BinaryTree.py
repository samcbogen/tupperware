...
class Node():
    ...
    Given a node t, you can visualize
    the node by running str(t) in the python interpreter.
...
class BinaryTree():
    ...
    def print_tree(self, traversal_type):
        ...
        of tree traversals: preorder, inorder, and postorder.
    ...
    def to_list(self, traversal_type):
        ...
        But by returning the results
        we can also do more computations on the results if needed.
        Many of the test cases
        for more complicated tree functions rely on this to_list function,
    ...
    def height(self):
        ...
        Recall that the
        height is the maximum length from the root to a leaf node.
    ...
    def _height(node):
        ...
        if not node:
            return -1
        if node.left:
            left_count = BinaryTree._height(node.left)
        if node.right:
            right_count = BinaryTree._height(node.right)
